...
#
# Nothing runs this automatically. The agent locates it from the skill path the
# harness handed it and runs it once per machine. That is the whole setup
# ceremony, and it now does exactly one thing: put the record somewhere and
# write the address down.
#
# It does NOT register the session-start card hook. The plugin declares that
# itself, in hooks/hooks.json at the plugin root, so the hook arrives with the
# plugin and is resolved by the harness against whichever version is active.
# adr/0006 records why that moved out of here.
#
...
#
# The marked block is the single source of truth, and it is now the ONLY thing
# standing between an installed plugin and a card. The hook fires on every
# machine that has the plugin; without the block it finds no address, emits
# nothing and exits clean. So one edit still turns the whole thing off, and
# there is no settings file to go and find because nothing was written there.
#
