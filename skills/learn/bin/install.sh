...
# never moves, prefers the copy that was live at install time, falls back to the
# newest sibling in the same cache, and exits silently when it can find neither
...
BIN="$HERE"
ROOT="$PLUGIN_ROOT"
if [ ! -x "\$BIN/session-card.sh" ] && [ -n "\$ROOT" ] && [ -d "\$ROOT" ]; then
  newest=""
