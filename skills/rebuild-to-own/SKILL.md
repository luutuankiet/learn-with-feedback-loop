...
3. **The learner profile** - resolved by the sidecar (its own resolution order: beside the skills directory first, then any pinned path). A track's `learn/CHARTER.md` records which profile this track belongs to, and that pin wins. Missing (new learner) - seed from the `PROFILE.template.md` shipped in this repo, ask the learner where it should live (OUTSIDE any single target repo - it spans tracks, and it belongs on a private remote), record the path in CHARTER. Read it per the sidecar's discipline, never whole.
...
The skill ships in the **learn-with-reps repo** (`skills/rebuild-to-own/`), alongside the mentoring discipline and its profile sidecar - never inside the target codebase. A teammate copies the three skill dirs into their own agent's skills (or symlinks them - a symlinked skill directory keeps the runtime copy and the tracked author copy from drifting), seeds a profile from `PROFILE.template.md`, then runs TAILOR against whatever repo they want to own. Per-user pointers to active tracks (host, repo path, syllabus worktree, profile path) live in a `pilots.md` next to this file - gitignored in the shared repo, since it is per-user state, not part of the template. `templates/pilots.md` is the seed for it, and `templates/` also carries the four `learn/` doc skeletons a new track is stamped out from.
